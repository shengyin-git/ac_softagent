...
## original cost
# def get_cost(args): #args init_state, action_trajs, env_class, env_kwargs
...
def get_cost(args):
    ...
    for i in range(N):
        ...
        costs.append(-max(reward_his)*100)
        # print('get_cost {}: {}'.format(i, ret))
    return costs
...
class ParallelRolloutWorker(object):
    ...
    def cost_function(self, init_state, action_trajs):
        ...
        #     type(init_state)
        #     args = {'init_state':init_state, 'action_trajs':splitted_action_trajs[i], 'env_class':self.env_class, 'env_kwargs': self.env_kwargs}
        #     # args = dict(init_state = init_state, action_trajs = splitted_action_trajs[i], env_class = self.env_class, env_kwargs = self.env_kwargs)
        #     ret = get_cost(**args)
