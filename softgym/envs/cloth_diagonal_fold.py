...
class ClothDiagonalFoldEnv(ClothEnv):
    def __init__(self, key_point_idx=None, cached_states_path='cloth_diagonal_fold_init_states.pkl', **kwargs):
    ...
    def diagonal_split(self, particle_grid_idx, flipped = False):
        dim = np.shape(particle_grid_idx)
        dim_x = dim[0]
        dim_y = dim[1]
        ...
        if flipped:
            for i in range(dim_x):
                ...
                for j in range(index_num):
                    output_index.append(particle_grid_idx[j,i])
        else:
            for i in range(dim_y):
                index_num = dim_x - i
                for j in range(index_num):
                    output_index.append(particle_grid_idx[i,j])
    ...
    def _reset(self):
        ...
        self.fold_group_a = self.diagonal_split(particle_grid_idx, flipped = False)
        flipped_particle_grid_idx = np.flipud(np.fliplr(particle_grid_idx))
        self.fold_group_b = self.diagonal_split(flipped_particle_grid_idx, flipped = True)  
    ...
    def compute_reward(self, action=None, obs=None, set_prev_reward=False, original_reward = True):
        """
    ...
    def _get_goal_state(self, camera_height, camera_width):
        ...
        particle_grid_idx = np.array(list(range(num_particles))).reshape(config['ClothSize'][1], config['ClothSize'][0])  # Reversed index here
        fold_group_a = self.diagonal_split(particle_grid_idx, flipped = False)
        flipped_particle_grid_idx = np.flipud(np.fliplr(particle_grid_idx))
        fold_group_b = self.diagonal_split(flipped_particle_grid_idx, flipped = True)  
        ...
        goal_pos = pyflex.get_positions().reshape([-1, 4])[:,:3]

        return goal_img, goal_pos#curr_pos[:,:3]

    def _get_flat_state(self, camera_height, camera_width):
        ...
        self.update_camera('default_camera', default_config['camera_params']['default_camera']) 
        self.action_tool.reset([0., -1., 0.]) # hide picker
        flat_img = self.get_image(camera_height, camera_width)
        return flat_img, initial_pos
