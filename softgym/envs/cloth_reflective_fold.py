...
class ClothReflectiveFoldEnv(ClothEnv):
    def __init__(self, key_point_idx=None, cached_states_path='cloth_reflective_fold_init_states.pkl', **kwargs):
    ...
    def _get_goal_state(self, camera_height, camera_width):
        ...
        curr_pos = pyflex.get_positions().reshape((-1, 4))
        curr_pos[fold_group_b, :] = curr_pos[fold_group_c, :].copy()
        curr_pos[fold_group_b, 1] += 0.00625*2
        
        curr_pos[fold_group_a, :] = curr_pos[fold_group_b, :].copy()
        curr_pos[fold_group_a, 1] += 0.00625*2
