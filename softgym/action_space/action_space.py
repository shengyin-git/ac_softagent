...
class Picker(ActionToolBase):
    ...
    def reset_action_space(self, key_point_index):        
        ## piking key point only
        ...
        index = np.linspace(0, self.num_key_points + 1, self.num_key_points+2)
        action_range = np.linspace(0, 1, self.num_key_points + 2)
        self.f_interp = interpolate.interp1d(action_range, index)
        self.none_threshold = self.num_key_points
    ...
    def step(self, action):
        ...
        ## This if for giving the picked particle index directly or by interpolation
        for i in range(self.num_picker):
            ...
            if picking_index >= self.none_threshold:  #math.isnan(action[i,3]):
                ...
                #     max_wait_step = 50
                #     for kk in range(max_wait_step):
                #         pyflex.step()
                #         curr_vel = pyflex.get_velocities()
                #         # print(max(abs(curr_vel)))
                #         if max(abs(curr_vel)) < stable_vel_threshold:
                #             new_particle_pos = np.array(pyflex.get_positions()).reshape(-1, 4)
                #             break                    
                #     self.picked_particles[i] = picked_particle
                self.picked_particles[i] = picked_particle
            
                sudo_picker_pos = particle_pos[picked_particle, :3]
                sudo_new_picker_pos = self._apply_picker_boundary(sudo_picker_pos + action[i, :3])
                new_picker_pos[i] = sudo_new_picker_pos
                # new_picker_pos[i, :] = self._apply_picker_boundary(picker_pos[i, :] + action[i, :3])
            
                new_particle_pos[self.picked_particles[i], :3] = particle_pos[self.picked_particles[i], :3] + sudo_new_picker_pos - sudo_picker_pos
                ...
                new_picker_pos[i,1] = new_picker_pos[i,1] + self.particle_radius + self.picker_radius
